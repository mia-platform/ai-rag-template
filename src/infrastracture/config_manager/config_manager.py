...
class ConfigManager(Generic[T]):
    ...
    def load_config_file(self):
        try:
            ...
            self.logger.error(f"Error on loading the config from {self.config_path}")
            raise err

    def load_config_schema_file(self):
        try:
            with open(self.config_schema_path, encoding="UTF-8") as file:
                self.schema = json.load(file)
        except FileNotFoundError as err:
            self.logger.error(f"Error on loading the config schema file from {self.config_schema_path}")
            raise err
    ...
    def validate_configuration(self):
        try:
            ...
            raise ConfigValidationError(f"Configuration validation against schema failed: {err.message}")
